...
const CLAUDE_RATE_LIMIT_RE = /(?:rate.limit|rate_limit|over.?capacity|too.many.requests|overloaded|\b429\b|resource.exhausted)/i;
...
export function detectClaudeRateLimit(input: {
  parsed: Record<string, unknown> | null;
  stdout: string;
  stderr: string;
}): { rateLimited: boolean } {
  ...
  const rateLimited = messages.some((line) => CLAUDE_RATE_LIMIT_RE.test(line));
  return { rateLimited };
}
